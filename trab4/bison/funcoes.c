...
tbl_funcoes* insert_fnc(char* nome, unsigned linha_decl, int aridade, unsigned retorno, int declarada, tbl_variaveis* fparams) {
    ...
    if (elem_found == NULL) {
        ...
        if (aridade > 0) {
            new_func->params = malloc(aridade*sizeof(Node*));
            new_func->tipos_esperados = malloc(aridade*sizeof(unsigned));
        }
        else {
            new_func->params = NULL;
            new_func->tipos_esperados = NULL;
        }

        for (i = 0; i < aridade; i++) {
            new_func->tipos_esperados[i] = fp->tipo;
            new_func->params[i] = create_node(fp->nome, 0, VARIAVEIS, 0);
            fp = fp->prox;
        }
        ...
        elem_found->params = malloc(aridade*sizeof(Node*));
        int i;
        //Define os tipos esperados como argumentos
        tbl_variaveis* fp = fparams;
        for (i = 0; i < aridade; i++) {
            elem_found->tipos_esperados[i] = fp->tipo;
            elem_found->params[i] = create_node(fp->nome, 0, VARIAVEIS, 0);
            fp = fp->prox;
        }
        //Insere os parâmetros da função na hash de variáveis da função
...
void insert_block(char* funcao, Node* block) {
  tbl_funcoes* fnc = lookup_fnc(funcao);
  fnc->block = block;
}
